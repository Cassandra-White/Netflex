...
router.post('/', function(req, res, next) {
	if (req.session && req.session._id) {
		...
		if (!post.login) {
			res.status(300).json({login: 'default / noLogin'});
		}
		else {
			collection.findOne(
				{ $or:
					...
					if (!result) {
						...
						let mailOptions = {
							from: '"Netflex" <netflex.test.mail@gmail.com>',
							to: result.email,
							subject: 'Netflex - Nouveau mot de passe',
							text: 'Voici ton nouveau mot de passe : ' + newPassword,
							html: '<b>Voici ton nouveau mot de passe : </b>' + newPassword + "<b> N'oubliez pas de changer votre mot de passe une fois connecté ;) </b>",
