...
router.post('/', function(req, res, next) {
	...
	if (login === undefined || firstName === undefined || lastName === undefined || email === undefined) {
		...
		if (login.length > 0 && login.length < 6) {
			errors.login = 'default login trop court';
		}
		if (login.length > 20) {
			errors.login = 'default login trop long';
		}
		if (login.length > 0 && !(/^[a-zà-ÿ0-9]+$/i.test(login))) {
			errors.login = 'default le login contient des caractere speciaux';
		}

		if (email.length> 0 && email.length < 4) {
			errors.email = 'default email trop court';
		}
		if (email.length > 50) {
			errors.email = 'default email trop long';
		}
		if (email.length > 0 && !(/^.+@.+\..+$/.test(email))) {
			errors.email = 'default contient des caractere speciaux ';
		}

		if (firstName.length> 0 && firstName.length < 1) {
			errors.firstName = 'defaul trop court'
		}
		if (firstName.length > 20) {
			errors.firstName = 'default trop long'
		}
		if (firstName.length > 0 && !(/^[a-zà-ÿ ]+$/i.test(firstName))) {
			errors.firstName = 'default utilisation de caractere speciaux'
		}

		if (lastName.length> 0 && lastName.length < 1) {
			errors.lastName = 'default trop court'
		}
		if (lastName.length > 20) {
			errors.lastName = 'default trop long'
		}
		if (lastName.length > 0 && !(/^[a-zà-ÿ ]+$/i.test(lastName))) {
			errors.lastName = 'default caractere speciaux'
		}
	}
